...
const upsertPost = async (
  ...
  await client.query(
    `
      INSERT INTO posts (slug, title, thumbnail, tags, content, views, "heartCount", "readingTime")
      VALUES ($1, $2, $3, $4, $5::jsonb, 0, 0, $6)
      ON CONFLICT (slug)
      DO UPDATE SET
...
async function run() {
  ...
  if (outputDir) {
    await fs.mkdir(resolve(process.cwd(), outputDir), { recursive: true });
  }
  ...
  for (const file of files) {
    ...
    if (outputDir) {
      const outPath = resolve(process.cwd(), outputDir, `${slug}.json`);
      await fs.writeFile(outPath, JSON.stringify(payload, null, 2), 'utf-8');
    }

    if (!dryRun) {
      await upsertPost(client, payload);
    }

    console.log(
      `[markdown-import] ${dryRun ? 'converted' : 'upserted'}: ${slug} (${basename(file)})`,
    );
  }
